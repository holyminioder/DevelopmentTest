        ...
        (JNIEnv * env, jobject)
{
    ...
    if(returnValueInt != PRINTER_NO_ERROR && returnValueInt != 0x0B) {
        ...
        LOGI("Init return %d, error str=%s", returnValueInt,errStr);
        return false;
    }else{
        isInit = true;
    }
...
//    PPrinterIsReady();
    ...
    //begin = systick();
//    returnValueInt = PPrintIsComplete(5);//?��?????????
//    if(returnValueInt != PRINTER_NO_ERROR && returnValueInt != 0x0B)
//    {
...
int PrintBarcodeTicket(int codeType)
{
    ...
    returnValueInt = PPrintString("���<ȫ����������3>");//????????
    if(returnValueInt != PRINTER_NO_ERROR && returnValueInt != 0x0B)
    {
    ...
    returnValueInt = PPrintString("ֱ ѡ Ʊ");//????????
    if(returnValueInt != PRINTER_NO_ERROR && returnValueInt != 0x0B)
    {
